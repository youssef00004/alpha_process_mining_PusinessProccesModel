...
Output filenames are suffixed with the input filename (without extension). For `Input.xlsx`:

| File | Description |
|------|-------------|
| `alpha_algorithm_output_Input.txt` | Full step-by-step Alpha Algorithm trace (Steps 1–8) |
| `pert_chart_Input.png` | Event Network Diagram (PERT Chart), 300 DPI |
| `pert_chart_Input.pdf` | Same chart as PDF |
| `process_mining_report_Input.xlsx` | 4-sheet Excel report (see below) |
...
├── event_log.py               # Hardcoded event log (4 cases, matches Input.xlsx)
├── alpha_algorithm.py         # Alpha Algorithm implementation (van der Aalst, 2004)
├── pert_chart.py              # PERT / Event Network Diagram
├── automation.py              # Full pipeline (Excel in → reports out)
├── Input.xlsx                 # Assignment event log (4 cases, 12 activities)
...
![PERT Chart](pert_chart_Input.png)
